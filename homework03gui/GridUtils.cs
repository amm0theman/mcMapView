      ...
      grid.LayoutUpdated += (s, e2) =>
        {
          ...
          foreach (var presenter in presenters)
          {
            ...
            if (phantom != null)
            {
              ...
              foreach (FrameworkElement child in phantom.Children.Cast<UIElement>().ToList())
              {
          ...
          var childCount = 256;
            ...
            int counter = 0;
            // set the row property for each child
            for (int i = 0; i < 16; i++)
            {
                    for (int j = 0; j < 16; j++)
                    {
                            var child = grid.Children[counter] as FrameworkElement;
                            Grid.SetRow(child, i);
